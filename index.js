...
function favourite(element){
	...
	for(let i = 0; i< currentMovieStack.length; i++){
		if(currentMovieStack[i].id == id){
			let favouriteMoviesShakti = JSON.parse(localStorage.getItem("favouriteMoviesShakti"));
			
			if(favouriteMoviesShakti == null){
				favouriteMoviesShakti = [];
			}

			favouriteMoviesShakti.unshift(currentMovieStack[i]);
			localStorage.setItem("favouriteMoviesShakti", JSON.stringify(favouriteMoviesShakti));
...
goToFavouriteButton.addEventListener('click', ()=>{
	let favouriteMoviesShakti = JSON.parse(localStorage.getItem("favouriteMoviesShakti"));
	if(favouriteMoviesShakti == null || favouriteMoviesShakti.length < 1){
		showAlert("you have not added any movie to favourite");
		return;
	}

	currentMovieStack = favouriteMoviesShakti;
	renderList("remove");
})
...
function remove(element){
	...
	for(let i = 0; i<favouriteMoviesShakti.length; i++){
		if(favouriteMoviesShakti[i].id == id){
			continue;
		}
		newFavouriteMovies.push(favouriteMoviesShakti[i]);
	}
	
	localStorage.setItem("favouriteMoviesShakti", JSON.stringify(newFavouriteMovies));
